...
export interface GroupModification {
  /**
   * Бэк, 2026-07-14: строка, а не число. Для группы без разбиения на
   * подгруппы — исходный id группы (напр. "4674"). Для подгруппы,
   * получившейся разбиением одной синковой группы Poster на несколько —
   * составной `"<parentGroupId>:<subgroupId>"` (напр. "4674:12"), чтобы
   * гарантировать уникальность ключей. Использовать как есть, не приводить к числу.
   */
  id: string;
  name: string;
  icon?: string | null;
  ...
  isSizes?: boolean;
  /**
   * Бэк, 2026-07-14. `null` для обычной группы и для «остатка»
   * (нераспределённых позиций исходной группы). Строковый id родительской
   * синковой группы — если это подгруппа-разбиение. Позволяет при желании
   * визуально сгруппировать подгруппы одной исходной группы.
   */
